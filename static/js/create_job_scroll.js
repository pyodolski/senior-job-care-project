...
function goBack() {
  if (currentStep > 1) {
    ...
    window.location.href = window.MAIN_URL;
  }
}
...
function updateNextButton() {
  ...
  switch (currentStep) {
    ...
    case 2:
      isValid =
        selectedData.title.length > 0 && selectedData.categories.length > 0;
      break;
    case 3:
      if (selectedData.workPeriod === "단기") {
        ...
        const hasTime =
          flexibleTime || (startTimeSelect?.value && endTimeSelect?.value);
...
async function submitJob() {
  ...
  try {
    ...
    const response = await fetch(window.CREATE_JOB_URL, {
      method: "POST",
      body: formData,
    });
    if (response.ok) {
      alert("공고가 성공적으로 작성되었습니다!");
      window.location.href = window.JOB_LIST_URL;
    } else {
